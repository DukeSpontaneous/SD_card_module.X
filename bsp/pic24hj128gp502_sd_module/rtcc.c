...
void RTCC_TimeGet(RTCC_DATETIME * value)
{
	...
	if (checkValue)
	{
		...
		do
		{
			...
			if (value->second != tempValue.second)
			{
				continue;
			}
			if (value->minute != tempValue.minute)
			{
				continue;
			}
			if (value->hour != tempValue.hour)
			{
				continue;
			}
			if (value->day != tempValue.day)
			{
				continue;
			}
			if (value->month != tempValue.month)
			{
				continue;
			}
			if (value->year != tempValue.year)
			{
				continue;
			}
			if (value->weekday != tempValue.weekday)
			{
				continue;
			}
		} while (0);
