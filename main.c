...
{	
			...
			SDConfig.SD_type != SD_TYPE_NO_CARD)
	{
		...
		// ѕомимо прочего устанавливает курсор
		switch (FS_checkMyFAT32(&LogacalDriveInfo))
		{
			case MY_FAT32_ERROR_OK: // ≈сли формат нормальный, то можно продолжать работать с ним:				
				break;
			case MY_FAT32_ERROR_FORMAT: // ≈сли формат имеет отклонение, то пытаемс€ отформатировать:				
				if (FS_formattingFAT32ToMyFAT32(&LogacalDriveInfo) != MY_FAT32_ERROR_OK)
					return false; // TODO: инициализаци€ завершена аварийно!
				break;
...
void main(void)
{
	...
	AD1PCFGL = 0xFFFF; // конфигураци€ цифровые I\O
	...
	SPI_initial(); // инициализаци€ SPI дл€ флешки
	//    T1initial(); // таймер времени
	//    T2initial(); // RS-485
	//    T4initial(); // отстутствие св€зи
	//    RS485_initial(); // инициализаци€ порта UART
	...
	{
		Nop(); // TODO: инициализаци€ завершена аварийно!
	}
